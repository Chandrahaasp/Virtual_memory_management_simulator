...
{  
    ...
    bool checkInRam = false; // Initial flag
    list<Page>::iterator iter;// It iterates through a list of pages in RAM
    for (iter = ram.begin(); iter != ram.end(); iter++)
    {
        ...
        if(actualPageFrame == pageFrame)   // PageFrame is in RAM
        {
            ...
            {
                if(tables[actualProcessNumber].getPresentBit(pageNumber) == 1) // Checks the present bit
                {
                    if(algo == 1)   // lru
                    {
                        tables[actualProcessNumber].setReferenceBit(pageNumber); // updates reference bits
                        tables[actualProcessNumber].setPresentBit(pageNumber); 
                       
                        ram.erase(iter);
                        ram.push_front(actualPage); //moves the page to the front of the RAM list
                    }
                    //cout << "lru" << endl;   
                    return true;
                }
                ...
                {
                    //Page fault handling
                    writeMemory(*iter); // It writes the existing page to memory
                    ram.erase(iter);
                    readMemory(actualPage);  //loads the requested page from memory
                    writeRam(actualPage); //and inserts it into RAM
                    break;
                }
            }
            else
            {
                ...
                if(ram.size() == ramSize)
                    popPage(); // If the page is found but belongs to a different process
         
                break;
            }
        }
    }
    if(checkInRam == false && ram.size() != 0) // Page not found in ram
    {
        readMemory(actualPage); //It reads the page from memory (presumably disk)
        if(operation == "read")
            operatingSystem(actualPage); // for further processing
        if(operation == "write")
            writeRam(actualPage);
    }
    
    if(ram.size() == 0)
        writeRam(actualPage);//If RAM is empty, it directly writes the page into RAM
